...
CLIProxyAPI 的核心调度思路在这里：
  - 每个 backend 维护一个账号列表
  - Round-robin 选号，跳过冷却中的账号
  - 请求失败时按错误类型决定冷却时长：
    * 配额/频率限制 → 长冷却（默认 60s）
    * 瞬时错误（超时/5xx）→ 短冷却（默认 15s）
  - 全部账号冷却时抛 RuntimeError（调用方可重试或报错）
"""
...
class AccountPool:
    ...
    def pick(self, backend: str) -> Optional[Account]:
        """Round-robin 选出一个可用账号；全部冷却中则返回 None。"""
        self._ensure_loaded()
        with self._lock:
            ...
            for offset in range(n):
                idx = (start + offset) % n
                acc = accs[idx]
                if not acc.is_cooling and acc.enabled:
                    # 下次从下一个开始，实现 round-robin
    ...
    def mark_failed(self, account: Account, exc: Optional[BaseException] = None) -> None:
        ...
        elif exc is not None and _is_transient_error(exc):
